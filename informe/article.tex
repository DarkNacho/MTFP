...
The source code for the experiments and heuristic implementations is available at \url{https://github.com/DarkNacho/MTFP}.
...
\end{itemize}
In all cases, the sociometric matrix density (positive relationships) was set to 30\%.
...
\subsubsection*{Discussion on $\text{N}^1$ Exploitation and Dominance of SLS}
The SLS's superior performance in large-scale instances \textit{suggests} that the MTFP landscape favors persistent exploitation of the $N^1$ neighborhood. The underperformance of VNS and GA \textit{may be attributed} to inefficient use of this crucial operator. \textbf{VNS} suffered from \textit{superficial exploitation}, limiting LS to $50$ NFE per shake, which likely prevented deep convergence. Conversely, the \textbf{GA} exhibited \textit{diluted exploitation}, as $N^1$ (via mutation) was applied sporadically, with most NFE consumed by high-overhead crossover/population management. The results strongly suggest that dedicating the majority of the NFE budget to successive $N^1$ movements \textit{could be the most efficient} strategy for the MTFP, validating SLS as the most scalable framework observed.
